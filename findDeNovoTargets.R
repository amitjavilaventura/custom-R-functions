...
findDeNovoTargets <- function(x, y){
  ...
  genes_target_denovo_all <- bind_rows(genes_target_denovo_prom,
                                       genes_target_denovo_dist) %>%
    unique()
  ...
  genes_target_common_all <- bind_rows(genes_target_common_prom,
                                       genes_target_common_dist) %>%
    unique()
  ...
  ## Targeted by promoter + distal
  genes_target_notarget_all <- bind_rows(genes_target_notarget_prom,
                                         genes_target_notarget_dist) %>%
    unique()
