...
#' 
#' 
filterAnno2 <- function(df){
  anno <- sub(" \\(.*\\)", "", df@anno$annotation)
  
  # change the annotation name of all the fields that ARE NOT "Promoter" to "Distal"
  ...
  # return annotation with only promoter and distal
  return(anno)
}
...
#' @param fill.colors. Character vector of length 2. Colors to use on each of the regions. Default c("Steelblue", "Lightsalmon3")

pieAnno2 <- function(df, main = NULL, size.main = 13, sub = NULL, size.sub = 11, legend = TRUE, size.legend = 9 , pos.legend = "bottom", fill.colors = c("Steelblue", "Lightsalmon3"), labels = TRUE, decimal = 3, size.label = 9){
  ...
  peak.num <- as.data.frame(c(length(prom), length(dist)))
  colnames(peak.num) <- "Number of peaks"
  rownames(peak.num) <- c("Promoter", "Distal")
  ...
  df.100 <- as.data.frame(matrix(c(prom.100,dist.100), ncol = 1))
  df.100$Category <- c("Promoter", "Distal")
  ...
  p <- ggplot(data = df.100, aes(x = "", y = Per100, fill = Category)) +
    ...
    geom_bar(width = 1, stat="identity", color = "gray15", show.legend = legend) +
  ...
  if(labels==TRUE){
    p <- p +  geom_text(aes(y = Per100/2 + c(0, cumsum(Per100)[-length(Per100)]),
                            label = c(round(Per100[1], digits = decimal), round(Per100[2], digits = decimal))), size = size.label)
  }
