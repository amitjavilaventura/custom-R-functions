...
#'
#'
signals_in_1region <- function(bigwigs, regions, names, names_order = names, operation = "mean", merge = T){
  ...
  for(i in 1:length(bigwigs)){
    ...
    coverage <- get_coverage(bigwigs[i], regions, op = operation)  %>%
      as_tibble() %>% mutate(signal_from = names[i])

    list[[names[i]]] <- coverage
  }

  if(merge){ list <- bind_rows(list) %>% mutate(signal_from = factor(signal_from, levels = names_order)) }
